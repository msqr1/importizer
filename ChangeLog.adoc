...
* Build & cache LibTooling for developers and CI.
* Official supported OS & architectures
* Major internal changes about organization & build system
...
* Removed all `*Action` return values from functions; actions are now performed directly using
lambda expressions.
...
* Improved `CondMinimizer` to correctly handle nested `#if` directives (it
previously looked for the earliest `#endif`).
* Simplified the condition hierarchy by removing conditions for all imports.
* Updated dependencies: bumped libfmt to 11.1.3, PCRE2 to 10.45-RC1, andArgparse to 3.2.
